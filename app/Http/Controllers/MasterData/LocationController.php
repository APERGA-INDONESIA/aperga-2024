...
class LocationController extends Controller
{
    ...
    {
        define('DOMAIN', "items");
        define('VIEW', "masterdata.location");
        define('URL', "masterdata/location");
        $this->limit = 10;
        $this->data = null;
        $this->data_edit = null;
    ...
    public function postCreate(Request $request)
    {
        ...
        if ($data_exists > 0) {
            $return_data['message'] = "Lokasi '{$request->name}' sudah ada, coba yang lain!";
    ...
    public function postEdit($id, Request $request)
    {
        ...
        if ($request->name != $this->data_edit->name) {
            $data_exists = $this->model->where(["name" => $request->name])
                ->count();
            if ($data_exists > 0) {
                return view(VIEW . '.edit', [
                    "message" => "Lokasi '{$request->name}' sudah ada, coba yang lain!",
                    DOMAIN => $this->data_edit,
                    "url" => URL,
                    "view" => VIEW,
                ]);
        ...
        $this->data_edit->name = $request->name;
        $this->data_edit->save();
        return redirect("/" . URL);
    }
